/*
 * Created by houyujie on 2018/9/15.
*/
/* eslint-disable */
let idTmr;
...
const export2Excel = (theadData, tbodyData, dataname) => {
	...
	for (let i = 0; i < tb_len; i++) {
		...
		for (let key in row) {
			if (re.test(row[key])) { // 如果为图片，则需要加div包住图片
				//
				tbody += '<td style="white-space: nowrap;width:' + width2 + 'px; height:' + height2 + 'px; text-align: center; vertical-align: middle;"><div style="display:inline;"><img src=\'' + row[key] + '\' ' + ' ' + 'width=' + '\"' + width + '\"' + ' ' + 'height=' + '\"' + height + '\"' + '></div></td>'
			} else {
				tbody += '<td style="white-space: nowrap; text-align: center;">' + row[key] + '</td>'
			}
		}
		tbody += '</tr>'
	}
	...
	// 导出表格，table为表格，dataname为Excel表名字；
